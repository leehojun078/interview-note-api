...
**단계**:
1. 사용자가 네비게이션 바에서 "리뷰 이력" 클릭
2. 리뷰 이력 목록 페이지(`/reviews`)로 이동
3. 이력 카드 목록 확인 (최신순):
